...
function slugify(name: string): string {
  return name.toLowerCase().trim().replace(/\s+/g, "-").replace(/[^a-z0-9-ąćęłńóśźż-]/g, "");
}
...
async function importData() {
  ...
  const statuses: [string, string, boolean][] = [
    ["Dostępny", "dostepny", true], ["W użyciu", "w-uzyciu", true],
    ["W naprawie", "w-naprawie", true], ["Uszkodzony", "uszkodzony", true],
    ["Wypożyczony", "wypozyczony", true], ["Zarezerwowany", "zarezerwowany", true],
    ["Zarchiwizowany", "zarchiwizowany", true], ["Oczekuje zatwierdzenia", "oczekuje-zatwierdzenia", true],
  ];
  ...
  // 4. Lookup maps
  console.log("🔄 Lookups...");
  const [typeRows] = await conn.query("SELECT id, legacy_type_id FROM categories WHERE legacy_type_id IS NOT NULL") as any;
  const typeMap = new Map<number,number>();
  for (const r of typeRows) typeMap.set(r.legacy_type_id, r.id);

  const [locRows] = await conn.query("SELECT id, legacy_room_id FROM locations WHERE legacy_room_id IS NOT NULL") as any;
  const locMap = new Map<number,number>();
  for (const r of locRows) locMap.set(r.legacy_room_id, r.id);

  const [statusRows] = await conn.query("SELECT id, slug FROM item_status") as any;
  const statusBySlug = new Map<string,number>();
  for (const r of statusRows) statusBySlug.set(r.slug, r.id);

  const [prodRows] = await conn.query("SELECT id, nazwa FROM inv_producenci") as any;
  ...
  let imported = 0, skipped = 0, borrowed = 0;
  for (const d of devices) {
    ...
    try {
      const [existing] = await conn.execute("SELECT id FROM items WHERE legacy_item_id=?", [d.id]);
      if (Array.isArray(existing) && (existing as any[]).length > 0) {
        await conn.execute(
          ...
          [d.serial||null, d.nr_inw||null, desc, name, manufacturer, d.model_name, catId, statId, locId, d.id]
        );
      } else {
        await conn.execute(
          `INSERT INTO items (name,manufacturer,model,serial,inventory_number,description,
           ...
           VALUES (?,?,?,?,?,?,?,?,?,?)`,
          [name, manufacturer, d.model_name, d.serial||null, d.nr_inw||null, desc, catId, statId, locId, d.id]
        );
      }
      imported++;
    } catch (e: any) {
  ...
  console.log(`✅ Done: ${imported} items (${skipped} errs), ${borrowed} borrowed, ${typeMap.size} types, ${locMap.size} rooms`);
  await conn.end();
}
