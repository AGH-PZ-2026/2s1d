...
async function runAutoMigrate(rawDb: RawDb): Promise<void> {
  ...
  const statements = [
    ...
    `CREATE TABLE \`users\` (\`id\` int AUTO_INCREMENT NOT NULL, \`email\` varchar(255) NOT NULL, \`hashed_password\` varchar(255), \`google_id\` varchar(255), \`auth_provider\` enum('local','google') NOT NULL DEFAULT 'local', \`is_active\` boolean NOT NULL DEFAULT true, \`is_approved\` boolean NOT NULL DEFAULT true, \`role\` enum('admin','user') NOT NULL DEFAULT 'user', CONSTRAINT \`users_id\` PRIMARY KEY(\`id\`), CONSTRAINT \`users_email_unique\` UNIQUE(\`email\`))`,
