...
router.get("/me", async (c) => {
  const db = c.get("db");
  const rows = await db.select({ id: users.id, email: users.email, role: users.role, isActive: users.isActive, isApproved: users.isApproved }).from(users).where(eq(users.id, c.get("userId"))).limit(1);
...
router.patch("/:id/approve", async (c) => {
  if (c.get("userRole") !== "admin") forbidden("Only admins can approve users");
...
router.patch("/:id/role", zValidator("json", updateRoleSchema), async (c) => {
  ...
  const existing = await db.select().from(users).where(eq(users.id, id)).limit(1);
  if (existing.length === 0) notFound("User not found");
  await db.update(users).set({ role: c.req.valid("json").role }).where(eq(users.id, id));
  const updated = await db.select({ id: users.id, email: users.email, role: users.role, isActive: users.isActive, isApproved: users.isApproved }).from(users).where(eq(users.id, id)).limit(1);
  return c.json(updated[0]);
});
