...
router.get("/scan/:qrData", async (c) => {
  ...
  const rows = itemId
    ...
    : await db.select().from(items).where(eq(items.systemId, parsed.systemId ?? "")).limit(1);

  if (rows.length === 0) notFound("Item not found");
  const item = rows[0];
  return c.json({ id: item.id, system_id: item.systemId, name: item.name, description: item.description, qr_data: qrData });
});
