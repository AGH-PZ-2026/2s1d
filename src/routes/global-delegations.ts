...
router.get("/", async (c) => {
  ...
  const uIds = [...new Set(rows.map((r) => r.userId).filter((v): v is number => v !== null))];
  const gIds = [...new Set(rows.map((r) => r.groupId).filter((v): v is number => v !== null))];
...
router.delete("/:id", async (c) => {
  ...
  if (userRole !== "admin") {
    const delegation = await db.select().from(delegations).where(eq(delegations.id, id)).limit(1);
    if (delegation.length === 0) return c.body(null, 404);
    
    const item = await db.select().from(items).where(eq(items.id, delegation[0].itemId)).limit(1);
