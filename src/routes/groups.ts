...
router.get("/:id", async (c) => {
  ...
  const m = await db.select().from(groupMembers).where(eq(groupMembers.groupId, id));
  return c.json(toResponse(rows[0], m.map((x) => x.userId)));
});

router.post("/", zValidator("json", createSchema), async (c) => {
  const db = c.get("db"); const body = c.req.valid("json");
  const exist = await db.select().from(groups).where(eq(groups.name, body.name)).limit(1);
  ...
  const created = await db.select().from(groups).where(eq(groups.id, result[0].insertId)).limit(1);
...
router.put("/:id", zValidator("json", createSchema), async (c) => {
  ...
  const exist = await db.select().from(groups).where(eq(groups.id, id)).limit(1);
  if (exist.length === 0) notFound("Group not found");
  const dup = await db.select().from(groups).where(and(eq(groups.name, body.name), ne(groups.id, id))).limit(1);
...
router.post("/:id/members", zValidator("json", addMemberSchema), async (c) => {
  const db = c.get("db"); const gid = Number(c.req.param("id")); const body = c.req.valid("json");
  const exist = await db.select().from(groups).where(eq(groups.id, gid)).limit(1);
  if (exist.length === 0) notFound("Group not found");
  const already = await db.select().from(groupMembers).where(and(eq(groupMembers.groupId, gid), eq(groupMembers.userId, body.userId))).limit(1);
  ...
  const m = await db.select().from(groupMembers).where(eq(groupMembers.groupId, gid));
  return c.json(toResponse(exist[0], m.map((x) => x.userId)), 201);
});

router.delete("/:id/members/:userId", async (c) => {
  const db = c.get("db"); const gid = Number(c.req.param("id")); const uid = Number(c.req.param("userId"));
  await db.delete(groupMembers).where(and(eq(groupMembers.groupId, gid), eq(groupMembers.userId, uid)));
  return c.body(null, 204);
});
