...
export default function BorrowingsPage() {
  ...
  const runAction = async (borrowing: Borrowing, action: 'approve' | 'reject' | 'handover') => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Nie udało się wykonać akcji wypożyczenia.');
    } finally {
    ...
    <div className="page">
      <div className="page-header">
        ...
        <div className="td-actions">
          <button className="btn btn-secondary" onClick={() => { setFormError(null); setModal({ mode: 'external' }); }} type="button">
            Wypożyczenie zewnętrzne
          </button>
          <button className="btn btn-primary" onClick={() => { setFormError(null); setModal({ mode: 'request' }); }} type="button">
            Nowy wniosek
          </button>
        </div>
      </div>
      ...
      {loading ? (
        <div className="loading-state"><div className="spinner" /><span>Ładowanie wypożyczeń...</span></div>
      ) : (
        <>
            ...
            {activeStatuses.map((status) => (
              <section className="borrowing-column" key={status}>
                <div className="borrowing-column__header">
                  ...
                  <span>{activeBorrowings.filter((borrowing) => borrowing.status === status).length}</span>
                </div>
                <div className="borrowing-column__list">
                  {activeBorrowings.filter((borrowing) => borrowing.status === status).map((borrowing) => (
                    <BorrowingCard
                      ...
                      onReturn={() => { setFormError(null); setModal({ mode: 'return', borrowing }); }}
                    />
                  ))}
                  {activeBorrowings.every((borrowing) => borrowing.status !== status) ? (
          ...
          <div className="section-header"><h2>Historia</h2></div>
          <table className="table">
            <thead>
              <tr>
                <th>ID</th><th>Przedmiot</th><th>Odbiorca</th>
                <th>Tryb</th><th>Status</th><th>Planowany zwrot</th><th>Utworzono</th>
              </tr>
            </thead>
            <tbody>
              {closedBorrowings.length === 0 ? (
                <tr><td colSpan={7}>Brak zakończonych wypożyczeń.</td></tr>
              ) : (
                closedBorrowings.map((borrowing) => (
                  <tr key={borrowing.id}>
                    ...
                    <td><span className={`badge badge-borrowing-${borrowing.status}`}>{statusLabels[borrowing.status]}</span></td>
      ...
      {modal ? (
        <div className="modal-overlay" onClick={() => setModal(null)}>
          <div className="modal" onClick={(event) => event.stopPropagation()}>
            ...
            {modal.mode === 'request' ? <BorrowingRequestForm items={items} loading={formLoading} onSubmit={handleRequest} /> : null}
            {modal.mode === 'external' ? <ExternalBorrowingForm items={items} loading={formLoading} onSubmit={handleExternal} /> : null}
            {modal.mode === 'return' ? <ReturnBorrowingForm borrowing={modal.borrowing} itemName={itemName(modal.borrowing.itemId)} loading={formLoading} onSubmit={(comment) => handleReturn(modal.borrowing, comment)} /> : null}
          </div>
        </div>
      ) : null}
    </div>
...
function BorrowingCard({ actionLoading, borrowerName, borrowing, itemName, onApprove, onHandover, onReject, onReturn }: {
  actionLoading: string | null; borrowerName: string; borrowing: Borrowing; itemName: string; onApprove: () => void; onHandover: () => void; onReject: () => void; onReturn: () => void;
}) {
  return (
    <article className="borrowing-card">
      <div className="borrowing-card__topline"><span>#{borrowing.id}</span><span className={`badge badge-borrowing-${borrowing.status}`}>{statusLabels[borrowing.status]}</span></div>
      <h3>{itemName}</h3>
      <dl><div><dt>Odbiorca</dt><dd>{borrowerName}</dd></div><div><dt>Tryb</dt><dd>{modeLabels[borrowing.mode]}</dd></div><div><dt>Planowany zwrot</dt><dd>{formatDate(borrowing.plannedReturnAt)}</dd></div></dl>
      <div className="borrowing-card__actions">
        {borrowing.status === 'pending' ? (<><button className="btn btn-sm btn-primary" disabled={actionLoading !== null} onClick={onApprove} type="button">{actionLoading === `${borrowing.id}:approve` ? 'Zapis...' : 'Zatwierdź'}</button><button className="btn btn-sm btn-danger" disabled={actionLoading !== null} onClick={onReject} type="button">Odrzuć</button></>) : null}
        {borrowing.status === 'reserved' ? (<button className="btn btn-sm btn-primary" disabled={actionLoading !== null} onClick={onHandover} type="button">{actionLoading === `${borrowing.id}:handover` ? 'Zapis...' : 'Wydaj'}</button>) : null}
        {borrowing.status === 'borrowed' ? (<button className="btn btn-sm btn-secondary" disabled={actionLoading !== null} onClick={onReturn} type="button">Zwrot</button>) : null}
      </div>
    </article>
  );
}

function BorrowingRequestForm({ items, loading, onSubmit }: { items: Item[]; loading: boolean; onSubmit: (payload: BorrowingRequestPayload) => void }) {
...
function ExternalBorrowingForm({ items, loading, onSubmit }: { items: Item[]; loading: boolean; onSubmit: (payload: ExternalBorrowingPayload) => void }) {
  const [itemId, setItemId] = useState(items[0]?.id ?? 0);
...
function ReturnBorrowingForm({ borrowing, itemName, loading, onSubmit }: { borrowing: Borrowing; itemName: string; loading: boolean; onSubmit: (comment: string) => void }) {
  const [comment, setComment] = useState('');
  return (
    <div className="form">
      <p className="return-summary">Zwrot wypożyczenia #{borrowing.id}: {itemName}</p>
      <label className="form-label">Komentarz zwrotu</label>
      <textarea className="form-input" value={comment} onChange={(event) => setComment(event.target.value)} placeholder="np. Bez uszkodzeń" />
      <div className="form-actions"><button className="btn btn-primary" disabled={loading} onClick={() => onSubmit(comment)} type="button">{loading ? 'Zapisywanie...' : 'Potwierdź zwrot'}</button></div>
    </div>
  );
}
