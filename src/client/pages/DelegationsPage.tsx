...
export default function DelegationsPage() {
  ...
  const handleDelete = async (delegation: Delegation) => {
    const label = delegation.user_id ? `użytkownika #${delegation.user_id}` : `grupę #${delegation.group_id}`;
  ...
  return (
    <div className="page">
      <div className="page-header"><div><h1 className="page-title">Delegacje przedmiotu</h1><p className="page-subtitle">Zarządzaj uprawnieniami do przedmiotu</p></div><button className="btn btn-primary" onClick={() => { setFormError(null); setShowModal(true); }}>+ Dodaj delegata</button></div>
...
function CreateForm({ onSubmit, loading }: { onSubmit: (p: CreateDelegationPayload) => void; loading: boolean }) {
  const [userId, setUserId] = useState(''); const [groupId, setGroupId] = useState(''); const [permission, setPermission] = useState<PermissionLevel>('edit');
  const submit = () => { if (!userId.trim() && !groupId.trim()) return; onSubmit({ user_id: userId ? parseInt(userId) : undefined, group_id: groupId ? parseInt(groupId) : undefined, permission }); };
  return (<div className="form">
    <label className="form-label">ID użytkownika</label><input className="form-input" value={userId} onChange={(e) => setUserId(e.target.value)} placeholder="np. 1" type="number" />
    <label className="form-label">ID grupy</label><input className="form-input" value={groupId} onChange={(e) => setGroupId(e.target.value)} placeholder="np. 1" type="number" />
    <p className="form-hint">Podaj ID użytkownika lub grupy.</p>
    <label className="form-label">Uprawnienie *</label><select className="form-input" value={permission} onChange={(e) => setPermission(e.target.value as PermissionLevel)}><option value="edit">Edycja</option><option value="manage">Zarządzanie</option></select>
    <div className="form-actions"><button className="btn btn-primary" onClick={submit} disabled={loading || (!userId.trim() && !groupId.trim())}>{loading ? 'Zapisywanie…' : 'Dodaj'}</button></div>
  </div>);
}
