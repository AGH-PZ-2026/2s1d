...
export default function BatchQrPage() {
  ...
  const [size, setSize] = useState<QrSize>('medium'); const [error, setError] = useState<string | null>(null);
  ...
  async function download() { setError(null); setIsDownloading(true); try { await batchQrService.download(selectedIds, size); } catch (err) { setError(err instanceof Error ? err.message : 'Nie udało się wygenerować PDF.'); } finally { setIsDownloading(false); } }

  return (
    <section>
      <div className="page-header"><div><h1 className="page-title">Drukowanie etykiet QR</h1><p className="page-subtitle">Wybierz przedmioty i rozmiar kodu, aby wygenerować zbiorczy PDF.</p></div><div className="td-actions"><select className="form-input" value={size} onChange={(e) => setSize(e.target.value as QrSize)}><option value="small">Mały</option><option value="medium">Średni</option><option value="large">Duży</option></select><button className="btn btn-primary" disabled={!selectedIds.length || isDownloading} type="button" onClick={download}>{isDownloading ? 'Generowanie...' : 'Pobierz PDF'}</button></div></div>
