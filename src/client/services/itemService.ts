...
export const itemService = {
  ...
  async updateLocation(itemId: number, locationId: number): Promise<void> {
    if (USE_MOCKS) { await delay(200); mockItems = mockItems.map((i) => i.id === itemId ? { ...i, locationId } : i); return; }
    const r = await fetch(`/api/v1/items/${itemId}`, { method: 'PATCH', headers: jsonAuthHeaders(), body: JSON.stringify({ locationId }) });
    await ensureOk(r);
  },
  async getGroups(): Promise<Group[]> {
    if (USE_MOCKS) { await delay(100); return [...mockGroups]; }
    const r = await fetch('/api/v1/groups/', { headers: authHeaders() }); await ensureOk(r);
    return ((await r.json()) as BackendGroup[]).map((g) => ({ id: g.id, name: g.name }));
  },
};
