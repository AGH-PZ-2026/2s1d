...
export const qrService = {
  async scan(qrData: string): Promise<ScannedQrItem> {
    const response = await fetch(`/api/v1/qr-codes/scan/${encodeURIComponent(qrData)}`, { headers: authHeaders() });
    await ensureOk(response); return response.json();
  },
  async getQuickActions(itemId: number): Promise<QuickActionItem> {
    const response = await fetch(`/api/v1/quick-actions/${itemId}`, { headers: authHeaders() });
    await ensureOk(response); return response.json();
  },
  async markDamaged(itemId: number): Promise<QuickActionItem> {
    const response = await fetch(`/api/v1/quick-actions/${itemId}/mark-damaged`, { method: 'PATCH', headers: authHeaders() });
    await ensureOk(response); return response.json();
  },
};
