...
export const statusService = {
  ...
  async update(id: number, payload: UpdateStatusPayload): Promise<Status> {
    ...
    if (status.type === 'system') throw new Error('Nie można edytować statusów systemowych.')
  ...
  async remove(id: number): Promise<void> {
    ...
    const status = mockStatuses.find(s => s.id === id)
    if (!status) throw new Error('Status nie istnieje.')
    if (status.type === 'system') throw new Error('Nie można usunąć statusów systemowych.')
