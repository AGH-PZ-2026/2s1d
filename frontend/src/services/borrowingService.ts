...
export const borrowingService = {
  ...
  async request(payload: BorrowingRequestPayload): Promise<Borrowing> {
    validateInternalBorrowing(payload.itemId, payload.mode);
    ...
    const response = await fetch('/api/v1/borrowings/requests', {
      method: 'POST',
  ...
  async createExternal(payload: ExternalBorrowingPayload): Promise<Borrowing> {
    ...
    const response = await fetch('/api/v1/borrowings/external', {
      method: 'POST',
      headers: jsonAuthHeaders(),
      body: JSON.stringify({
        ...payload,
...
function validateInternalBorrowing(
  ...
  if (mode === 'external') throw new Error('Użyj formularza zewnętrznego.');
}
...
async function patchStatus(
  ...
  if (USE_MOCKS) {
    ...
    const now = new Date().toISOString();
    if (action === 'approve') {
      return updateMockBorrowing(id, { status: 'reserved', approvedAt: now });
    }
    if (action === 'handover') {
      return updateMockBorrowing(id, {
        status: 'borrowed',
